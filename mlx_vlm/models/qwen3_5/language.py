...
def make_quantization_config(model_or_config: Any) -> dict | None:
    """Return the native MLX config for a supported Qwen FP8 checkpoint."""
    if not isinstance(model_or_config, dict):
        model_or_config = getattr(model_or_config, "config", model_or_config)
    if not isinstance(model_or_config, dict):
        return None

    quantization = model_or_config.get("quantization_config") or {}
    is_supported = (
        model_or_config.get("model_type") == "qwen3_5"
